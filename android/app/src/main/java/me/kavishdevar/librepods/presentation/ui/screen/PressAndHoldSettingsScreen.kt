...
import me.kavishdevar.librepods.presentation.ui.component.SelectItem
import me.kavishdevar.librepods.presentation.ui.component.StyledSelectList
...
fun LongPress(
    ...
    if (modesByte != null) {
        ...
        Log.d(
            "PressAndHoldSettingsScreen",
            "Transparency mode: ${(modesByte and 0x04) != 0.toByte()}"
        )
        Log.d(
            "PressAndHoldSettingsScreen",
            "Noise Cancellation mode: ${(modesByte and 0x02) != 0.toByte()}"
        )
    ...
    val prefKey =
        if (name.lowercase() == "left") "left_long_press_action" else "right_long_press_action"
    val longPressActionPref =
        sharedPreferences.getString(prefKey, StemAction.CYCLE_NOISE_CONTROL_MODES.name)
    Log.d(
        "PressAndHoldSettingsScreen",
        "Long press action preference ($prefKey): $longPressActionPref"
    )
    var longPressAction by remember {
        mutableStateOf(
            StemAction.valueOf(
                longPressActionPref ?: StemAction.CYCLE_NOISE_CONTROL_MODES.name
            )
        )
    }
...
//            Spacer(Modifier.height(spacerHeight))
            ...
            if (longPressAction == StemAction.CYCLE_NOISE_CONTROL_MODES) {
                ...
                val offListeningModeValue =
                    ServiceManager.getService()!!.aacpManager.controlCommandStatusList.find {
                ...
                var currentByte by remember { mutableStateOf(0) }
    ...
    Log.d(
        "PressAndHoldSettingsScreen", "Current byte: ${
            ServiceManager.getService()!!.aacpManager.controlCommandStatusList.find {
